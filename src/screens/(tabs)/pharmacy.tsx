...
export default function ProductListScreen() {
  ...
  const renderEmptyState = () => (
    <View style={tw`flex-1 justify-center items-center p-4`}>
      <Text style={tw`text-xl font-semibold text-blue-800 mb-2`}>No Products Found</Text>
      <Text style={tw`text-base text-blue-600 text-center`}>
        {searchQuery 
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-blue-50`}>
      ...
      {/* Search Bar */}
      <View style={tw`px-4 pb-2 pt-1 bg-blue-50 shadow-sm`}>
        <View style={[
          tw`flex-row items-center bg-blue-100 rounded-xl`,
          isSearchFocused && tw`bg-blue-50 border-2 border-blue-600`
        ]}>
          <View style={tw`px-3 py-2`}>
            <Search size={20} color={isSearchFocused ? "#164aa3ff" : "#6B7280"} />
          </View>
          <TextInput
            style={tw`flex-1 py-2 pr-3 text-base text-blue-800`}
        ...
        <View style={tw`flex-row items-center justify-between mt-2`}>
          ...
          <View style={tw`flex-row items-center gap-2`}>
            <TouchableOpacity 
              ...
              style={tw`flex-row items-center bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5`}
            >
              <ChevronDown size={16} color="#4B5563" />
              <Text style={tw`ml-1 text-sm text-blue-800`}>Sort</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={() => setFilterVisible(true)}
              style={tw`bg-blue-50 border border-blue-200 rounded-lg px-3 py-1.5`}
            >
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity 
              style={tw`flex-1 m-2 bg-blue-50 rounded-xl shadow-sm border border-blue-100`}
              onPress={() => {
            ...
            >
              ...
              />
              <View style={tw`p-3`}>
                <Text style={tw`text-lg font-medium text-blue-800`}>{item.name}</Text>
                <Text style={tw`text-sm text-blue-600`}>{item.brand}</Text>
                <Text style={tw`text-xs text-blue-600 mt-1`} numberOfLines={2}>
                  {item.description}
                </Text>
                <View style={tw`flex-row items-center justify-between mt-2`}>
      ...
      >
        <TouchableOpacity 
          style={tw`flex-1 bg-blue-900 bg-opacity-10`} 
          activeOpacity={1} 
          onPress={() => setSortVisible(false)}
        >
          <View style={tw`mt-auto bg-blue-50 rounded-t-3xl`}>
            <View style={tw`w-12 h-1 bg-blue-200 rounded-full mx-auto mt-3`} />
            <View style={tw`p-4`}>
              <Text style={tw`text-xl font-semibold text-blue-800 mb-4`}>Sort By</Text>
              <TouchableOpacity 
              ...
              >
                <Text style={tw`text-base ${sortBy === 'name' ? 'text-blue-600 font-medium' : 'text-blue-800'}`}>Name</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={tw`py-3 px-4 flex-row justify-between items-center`}
                onPress={() => {
