...
const LabTestCategoriesScreen = () => {
  ...
  const renderItem = ({ item }: { item: typeof categories[0] }) => (    <TouchableOpacity
      style={[
        tw`bg-blue-50 rounded-3xl py-4 px-2 items-center justify-center shadow-sm elevation-3 mb-3`,
        { width: '31%', marginHorizontal: '1%' }
      ]}
      activeOpacity={0.85}
      onPress={() => handleCategorySelect(item.name)}
    >
      <View style={[tw`w-12 h-12 rounded-2xl mb-2 bg-blue-100`, { overflow: 'hidden' }]}> 
        <Image
          source={item.image}
      ...
      <Text style={tw`text-[12px] font-medium text-center text-blue-800`} numberOfLines={2}> 
        {item.name}
      </Text>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={tw`flex-1 bg-blue-50`}>
      <PageHeader title="Lab Test Categories" backgroundColor="#164aa3ff" textColor="#fff" onBackPress={() => navigation.goBack()} />
      {/* Search Bar */}
      <View style={tw`flex-row items-center bg-blue-50 rounded-3.5xl px-3 py-2.5 mb-5 shadow-sm elevation-3 mx-4 mt-4`}>
        <Search size={20} color="#999" style={tw`mr-2`} />
        <TextInput
          style={tw`text-base text-blue-800 flex-1`}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={tw`text-center mt-12 text-base text-blue-400`}>
            No categories found.
          </Text>
        }
