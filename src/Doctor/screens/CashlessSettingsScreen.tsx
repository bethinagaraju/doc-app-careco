  ...
  return (
    <View style={tw`flex-1 bg-blue-50`}>
      ...
      <ScrollView contentContainerStyle={tw`p-4 pb-10`}>
        <Text style={tw`text-2xl font-bold text-blue-700 mb-2 text-center`}>Payment Settings</Text>
        <Text style={tw`text-base text-blue-600 mb-6 text-center`}>Manage your cashless payment options</Text>

        {/* Payment Methods */}
        <View style={tw`bg-blue-100 rounded-xl p-4 mb-4`}>
          <Text style={tw`text-lg font-bold text-blue-700 mb-4`}>Payment Methods</Text>
          
          <View style={tw`flex-row justify-between items-center mb-4`}>
            <View>
              <Text style={tw`text-blue-700 font-medium`}>Enable Cashless Payments</Text>
              <Text style={tw`text-blue-600 text-sm`}>Accept digital payments from patients</Text>
            </View>
            <Switch
              value={cashlessEnabled}
              onValueChange={setCashlessEnabled}
              trackColor={{ false: '#bbf7d0', true: '#059669' }}
              thumbColor={cashlessEnabled ? '#164aa3ff' : '#bbf7d0'}
            />
          </View>

          <View style={tw`flex-row justify-between items-center mb-4`}>
            <View>
              <Text style={tw`text-blue-700 font-medium`}>Auto Settlement</Text>
              <Text style={tw`text-blue-600 text-sm`}>Automatically settle payments daily</Text>
            </View>
            <Switch
              value={autoSettlement}
              onValueChange={setAutoSettlement}
              trackColor={{ false: '#bbf7d0', true: '#059669' }}
              thumbColor={autoSettlement ? '#164aa3ff' : '#bbf7d0'}
            />
          </View>

          <View style={tw`flex-row justify-between items-center`}>
            <View>
              <Text style={tw`text-blue-700 font-medium`}>Instant Refunds</Text>
              <Text style={tw`text-blue-600 text-sm`}>Process refunds immediately</Text>
            </View>
            <Switch
              value={instantRefunds}
              onValueChange={setInstantRefunds}
              trackColor={{ false: '#bbf7d0', true: '#059669' }}
              thumbColor={instantRefunds ? '#164aa3ff' : '#bbf7d0'}
            />
          </View>
        </View>
        ...
        {/* Bank Details */}
        <View style={tw`bg-blue-100 rounded-xl p-4 mb-4`}>
          <Text style={tw`text-lg font-bold text-blue-700 mb-4`}>Bank Details</Text>
          <TouchableOpacity 
        ...
        {/* Save Button */}
        <TouchableOpacity
          style={tw`bg-blue-600 rounded-lg py-4 items-center mx-4 mb-4`}
          onPress={() => {
