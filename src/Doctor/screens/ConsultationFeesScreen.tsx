  ...
  return (
    <View style={tw`flex-1 bg-blue-50`}>
      ...
      <ScrollView contentContainerStyle={tw`p-5 pb-10`}>
        <Text style={tw`text-2xl font-bold text-blue-700 mb-2 text-center`}>Your Fees</Text>
        <Text style={tw`text-base text-blue-600 mb-6 text-center`}>Manage your consultation charges</Text>

        {/* Video Consultation Fees */}
        <View style={tw`bg-blue-100 rounded-2xl p-5 shadow-sm mb-5`}>
          <View style={tw`flex-row justify-between items-center mb-4`}>
          <View style={tw`flex-row items-center`}>
            <Video size={24} color="#164aa3ff" />
            <Text style={tw`text-lg font-bold text-blue-700 ml-2`}>Video Consultation</Text>
            </View>
            <Switch
              value={isVideoEnabled}
              onValueChange={setIsVideoEnabled}
              trackColor={{ false: '#bbf7d0', true: '#059669' }}
              thumbColor={isVideoEnabled ? '#164aa3ff' : '#bbf7d0'}
            />
          </View>
          {editMode === 'video' ? (
            <View style={tw`bg-blue-50 p-4 rounded-xl mb-3`}>
              <View style={tw`flex-row justify-between items-center mb-2`}>
                <Text style={tw`text-blue-600`}>Regular Consultation</Text>
                <TextInput
                  style={tw`border border-blue-700 rounded px-2 py-1 w-20 text-right text-blue-700 font-bold bg-white`}
              ...
              <View style={tw`flex-row justify-between items-center`}>
                <Text style={tw`text-blue-600`}>Follow-up Visit</Text>
                <TextInput
                  style={tw`border border-blue-700 rounded px-2 py-1 w-20 text-right text-blue-700 font-bold bg-white`}
                  keyboardType="numeric"
                  value={videoFollowUp}
              ...
              <View style={tw`flex-row justify-end mt-3`}>
                <TouchableOpacity
                  style={tw`px-4 py-2 bg-blue-600 rounded-full mr-2`}
                  onPress={() => {
                    setEditMode('none');
                  }}
                >
                ...
                <TouchableOpacity
                  style={tw`px-4 py-2 bg-blue-200 rounded-full`}
                  onPress={() => {
                ...
                >
                  <Text style={tw`text-blue-700 font-bold`}>Cancel</Text>
                </TouchableOpacity>
              </View>
            </View>
          ) : (
            <View style={tw`bg-blue-50 p-4 rounded-xl mb-3`}>
              <View style={tw`flex-row justify-between items-center mb-2`}>
                <Text style={tw`text-blue-600`}>Regular Consultation</Text>
                <Text style={tw`text-blue-700 font-bold`}>₹{videoRegular}</Text>
              </View>
              <View style={tw`flex-row justify-between items-center`}>
                <Text style={tw`text-blue-600`}>Follow-up Visit</Text>
                <Text style={tw`text-blue-700 font-bold`}>₹{videoFollowUp}</Text>
              </View>
            </View>
          )}
          <TouchableOpacity style={tw`flex-row items-center justify-center mt-2`} onPress={() => setEditMode(editMode === 'video' ? 'none' : 'video')}>
            <BadgeDollarSign size={16} color="#164aa3ff" />
            <Text style={tw`text-blue-700 font-medium ml-1`}>{editMode === 'video' ? 'Close' : 'Edit Fees'}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={tw`bg-blue-100 rounded-2xl p-5 shadow-sm mb-5`}>
          <View style={tw`flex-row justify-between items-center mb-4`}>
          <View style={tw`flex-row items-center`}>
            <Users size={24} color="#164aa3ff" />
            <Text style={tw`text-lg font-bold text-blue-700 ml-2`}>In-Clinic Consultation</Text>
            </View>
            <Switch
              value={isInClinicEnabled}
              onValueChange={setIsInClinicEnabled}
              trackColor={{ false: '#bbf7d0', true: '#059669' }}
              thumbColor={isInClinicEnabled ? '#164aa3ff' : '#bbf7d0'}
            />
          </View>
          {editMode === 'inClinic' ? (
            <View style={tw`bg-blue-50 p-4 rounded-xl mb-3`}>
            ...
            <View style={tw`bg-blue-50 p-4 rounded-xl mb-3`}>
              <View style={tw`flex-row justify-between items-center mb-2`}>
                <Text style={tw`text-blue-600`}>Regular Consultation</Text>
                <Text style={tw`text-blue-700 font-bold`}>₹{clinicRegular}</Text>
              </View>
              <View style={tw`flex-row justify-between items-center`}>
                <Text style={tw`text-blue-600`}>Follow-up Visit</Text>
                <Text style={tw`text-blue-700 font-bold`}>₹{clinicFollowUp}</Text>
              </View>
            </View>
          )}
          <TouchableOpacity style={tw`flex-row items-center justify-center mt-2`} onPress={() => setEditMode(editMode === 'inClinic' ? 'none' : 'inClinic')}>
            <BadgeDollarSign size={16} color="#164aa3ff" />
            <Text style={tw`text-blue-700 font-medium ml-1`}>{editMode === 'inClinic' ? 'Close' : 'Edit Fees'}</Text>
          </TouchableOpacity>
        </View>

        {/* Additional Settings */}
        <View style={tw`bg-blue-100 rounded-2xl p-5 shadow-sm mb-5`}>
          <Text style={tw`text-lg font-bold text-blue-700 mb-4`}>Additional Settings</Text>
          <TouchableOpacity style={tw`flex-row items-center justify-between py-2`}>
            <Text style={tw`text-blue-600`}>Cancellation Policy</Text>
            <Calendar size={20} color="#164aa3ff" />
          </TouchableOpacity>
        </View>
        <TouchableOpacity
          style={tw`mt-2 bg-blue-600 rounded-full px-6 py-3 items-center`}
          activeOpacity={0.85}
          onPress={() => {
