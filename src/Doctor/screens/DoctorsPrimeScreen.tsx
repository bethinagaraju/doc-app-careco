		...
		<View style={tw`flex-1 bg-blue-50`}>
			...
			<ScrollView contentContainerStyle={tw`p-5 pb-10 items-center`}>
				<View style={tw`w-full max-w-xl bg-white rounded-2xl p-6 shadow-sm mb-6`}>
					<Text style={tw`text-3xl font-bold text-blue-700 mb-2 text-center`}>
						Doctor's Prime
					</Text>
					...
					<Text style={tw`text-lg font-bold text-blue-600 mb-2 text-center`}>Prime Features</Text>
					{features.map((f, idx) => (
						<View key={idx} style={tw`flex-row items-start mb-5`}>
							<View style={tw`mr-4 mt-1`}>{f.icon}</View>
							<View style={tw`flex-1`}>
								<Text style={tw`text-blue-700 font-semibold text-base`}>{f.title}</Text>
								<Text style={tw`text-blue-600 text-sm mt-1`}>{f.desc}</Text>
							</View>
						</View>
					))}
					<TouchableOpacity
						style={tw`mt-4 bg-blue-600 rounded-full px-6 py-3 items-center`}
						activeOpacity={0.85}
					>
				...
				<View style={tw`w-full max-w-xl bg-white rounded-2xl p-6 shadow-sm`}>
					<Text style={tw`text-lg font-bold text-blue-700 mb-2`}>Why Go Prime?</Text>
					<Text style={tw`text-blue-600 text-base mb-2`}>
						Doctor's Prime is designed for ambitious doctors who want to:
					</Text>
					<Text style={tw`text-blue-700 text-base mb-1`}>
